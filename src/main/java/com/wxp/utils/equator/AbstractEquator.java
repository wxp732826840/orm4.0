...
public abstract class AbstractEquator implements Equator {
    ...
    private List<EquatorStrategy> equatorStrategies = new ArrayList<>();
    ...
    protected boolean isFieldEquals(FieldInfo fieldInfo) {
        ...
        if (equatorStrategies.size() > 0) {
            for (EquatorStrategy equatorStrategy : equatorStrategies) {
                if (!equatorStrategy.setCustomeStrtegy(fieldInfo)) {
                    return false;
                }
            }
        }
    ...
    /***
     * 获取所有比较策略，并根据sort排序
     * @param equatorStrategy
     */
    public void setFiledStrategys(EquatorStrategy... equatorStrategy) {
        Arrays.stream(equatorStrategy).forEach(x -> equatorStrategies.add(x));
        if (equatorStrategies.size() > 0) {
            Collections.sort(equatorStrategies, (a, b) -> b.sort() - a.sort());
        }
    }
